...
function coordinatorWithResponses(map: Record<string, unknown>) {
  ...
  globalThis.fetch = (async (
    ...
    if (path in map)
      return new Response(JSON.stringify(map[path]), { status: 200 })
...
describe('SyncCoordinator workspace/chart pull', () => {
  it('pullWorkspaces hydrates a slot when the server copy is newer', async () => {
    ...
    const c = coordinatorWithResponses({
      ...
      '/api/user/workspace': {
        slots: [{ name: 'terminal-layout', panels: { v: 2 }, updatedAt: 5000 }],
      },
      '/api/user/chart-state': {
        pairKey: '_all',
        indicators: {},
        drawings: {},
        settings: {},
    ...
    expect(localStorage.getItem('pairlens:terminal.layout')).toBe(
      JSON.stringify({ v: 2 }),
    )
    unsub()
    c.destroy()
  ...
  it('flushTier2 sends updatedAt + writes sync-ts for a workspace slot', async () => {
    ...
    const putCall = fetchCalls.find(
      ([u, i]) =>
        String(u).includes('/api/user/workspace/terminal-layout') &&
        i?.method === 'PUT',
    )
    ...
    expect(
      localStorage.getItem('pairlens:sync-ts:terminal.layout'),
    ).toBeTruthy()
    c.destroy()
  })
})
